...
public class ParseUtil {
    ...
    public static long getSQLId(String stmt) {
        ...
        if (offset != -1 && stmt.length() > ++offset) {
            ...
            try {
                return Long.parseLong(id);  //将String类型的id解析成带符号的十进制
            ...
            if (c == '\\') {  //表示读取的是转义字符，对下一字节分析处理
                switch (c = stmt.charAt(++offset)) {
                    ...
                    case 't':
                        sb.append('\t');
                        break;
                    case 'Z':   // \Z代表文本的末尾 (char)26表示SUB字符，通常用于结束符或清除屏幕等
                        sb.append((char) 26);
                        break;
                    default:
                        sb.append(c);
    ...
    private static String parseIdentifierEscape(String stmt, int offset) {
        ...
        for (++offset; offset < stmt.length(); ++offset) {
            char c = stmt.charAt(offset);
            if (c == '`') { //Markdown 中用于表示行内代码的符号，两个反引号标记为代码
                if (offset + 1 < stmt.length() && stmt.charAt(offset + 1) == '`') {
                    ++offset;
                    sb.append('`');
                } else {
    ...
    /**
     * 查找SQL语句中的注释符号并忽略这段注释
     * @param stmt
     * @param offset
     * @return 返回注释结束的位置，以便在后续的解析中忽略这段注释
     */
